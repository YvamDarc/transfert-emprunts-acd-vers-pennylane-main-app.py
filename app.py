    ...
    if parsed is None:
        ...
        try:
            ...
            with st.expander("Paramètres de conversion", expanded=True):
                ...
                with first:
                    payment_label = st.selectbox(
                        ...
                        help=(
                            "Le premier choix conserve l'annuité ACD, puis ajoute l'assurance, la commission et les autres frais réels. "
                            "Les montants « Autre » reconnus comme déblocages sont automatiquement exclus. "
            ...
            if result.initial_rows_excluded:
                st.info(
                    f"La ligne ACD n°0 a été reconnue comme ligne d'ouverture et retirée de l'échéancier. "
                    f"Solde d'ouverture : {result.opening_balance:,.2f} €".replace(",", " ")
                )
            if not result.drawdowns.empty:
                st.success(
                    f"{len(result.drawdowns)} déblocage(s) détecté(s), pour un capital total de "
                    f"{result.total_drawdowns:,.2f} €.".replace(",", " ")
                )
                with st.expander(
                    "Voir le détail des déblocages successifs",
                    expanded=len(result.drawdowns) > 1,
                ):
                    st.dataframe(
                        result.drawdowns,
                        hide_index=True,
                        ...
                        column_config={
                            ...
                            "N° ACD": st.column_config.NumberColumn(format="%d"),
                            "Déblocage détecté (€)": st.column_config.NumberColumn(format="%.2f"),
                            "Capital cumulé débloqué (€)": st.column_config.NumberColumn(format="%.2f"),
                            "Solde après opération (€)": st.column_config.NumberColumn(format="%.2f"),
                        },
                    )
                    st.caption(
                        "Calcul : déblocage = nouveau restant dû − ancien restant dû + capital remboursé. "
                        "Si le même montant figure dans « Autre », il est traité comme un versement de la banque et non comme un frais."
                    )
            ...
            diagnostics = validate_pennylane_loan(
                edited_summary,
                edited_schedule,
                schedule_drawdowns=result.schedule_drawdowns,
            ...
            metric_1.metric("Capital total débloqué", f"{capital:,.2f} €".replace(",", " "))
...
with help_tab:
    ...
    st.markdown(
        ...
        Le modèle Pennylane ne possède pas de colonne « Déblocage ». L'application inscrit donc le **total de tous les
        versements bancaires** dans `Capital (€)` et conserve, ligne par ligne, le `Restant Dû` fourni par ACD.
        """
    )
    ...
    rules = pd.DataFrame(
        [
            ["Capital", "Tous les déblocages détectés", "Solde d'ouverture + chaque hausse du restant dû corrigée du capital remboursé."],
            ...
            ["Autres frais", "Commission + Autre hors déblocages", "Un montant Autre qui explique une hausse identique du solde est un versement, pas un frais."],
            ...
            ["Solde", "Restant Dû", "Contrôlé : solde précédent + déblocage - capital remboursé."],
            ["Différés", "Lignes entièrement à zéro", "Conservés par défaut pour respecter la chronologie du prêt."],
            ["Nombre d'échéances", "Nombre de lignes après la ligne n°0", "Inclut les déblocages et les échéances nulles conservées."],
        ],
    ...
    st.info(
        "Cas des versements successifs : si le prêt est débloqué en plusieurs fois, le capital Pennylane est la somme de tous les versements. "
        "L'application affiche leur détail avant l'export pour permettre un contrôle immédiat."
    )
