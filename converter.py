...
class ConversionResult:
    ...
    schedule_drawdowns: pd.Series
    opening_balance: float
    total_drawdowns: float
...
def convert_acd_loan(
    ...
    # ACD peut inscrire un premier déblocage dans « Autre » sur la ligne n°0.
    # Cette colonne n'est donc pas utilisée pour décider si la ligne n°0 est
    # une ligne d'ouverture plutôt qu'une échéance.
    opening_flow_columns = [column for column in money_columns if column != "Autre"]
    zero_opening_flow = _sum_columns(source, opening_flow_columns).abs() <= 0.005
    numbered_zero = pd.to_numeric(source["N°"], errors="coerce").fillna(-1).eq(0)
    initial_mask = numbered_zero & zero_opening_flow & source["Restant Dû"].notna()
    initial_rows_excluded = int(initial_mask.sum())

    if initial_mask.any():
        opening_balance = float(source.loc[initial_mask, "Restant Dû"].iloc[0] or 0.0)
    else:
        opening_balance = 0.0

    schedule_source = source.loc[~initial_mask].copy().reset_index(drop=True)

    # Reconstitution des déblocages :
    # solde courant = solde précédent + déblocage - capital remboursé.
    # Le calcul reste valable lorsqu'un déblocage et un remboursement ont lieu
    # sur la même ligne. Les écarts inférieurs à 5 centimes sont des arrondis.
    inferred_drawdowns: list[float] = []
    unexplained_decreases: list[tuple[int, float]] = []
    previous_balance = opening_balance
    for index, row in schedule_source.iterrows():
        balance = _number(row.get("Restant Dû")) or 0.0
        principal = _number(row.get("Capital remb.")) or 0.0
        balance_movement = balance - previous_balance + principal
        if balance_movement > 0.05:
            inferred_drawdowns.append(round(balance_movement, 2))
        else:
            inferred_drawdowns.append(0.0)
            if balance_movement < -0.05:
                unexplained_decreases.append((index + 1, balance_movement))
        previous_balance = balance

    schedule_source["Déblocage détecté (€)"] = pd.Series(inferred_drawdowns, dtype=float)

    # Dans les échéanciers à déblocages successifs, ACD inscrit généralement
    # le versement en colonne « Autre ». Lorsqu'il correspond à l'augmentation
    # du restant dû, ce n'est pas un frais et il ne doit pas être importé comme tel.
    other_amounts = schedule_source["Autre"].fillna(0).astype(float)
    drawdown_amounts = schedule_source["Déblocage détecté (€)"].fillna(0).astype(float)
    drawdown_in_other = (drawdown_amounts > 0.05) & ((other_amounts - drawdown_amounts).abs() <= 0.05)
    true_other_amounts = other_amounts.mask(drawdown_in_other, 0.0)
    schedule_source["Autre hors déblocage (€)"] = true_other_amounts

    actual_flow_columns = ["Annuité", "Intérêts", "Assurance", "Commission", "Capital remb."]
    zero_schedule = (
        _sum_columns(schedule_source, actual_flow_columns).abs() <= 0.005
    ) & (true_other_amounts.abs() <= 0.005) & (drawdown_amounts.abs() <= 0.05)
    ...
    other_fees = schedule_source["Commission"].fillna(0) + schedule_source["Autre hors déblocage (€)"].fillna(0)
    detailed_payment = (
    ...
    schedule_drawdowns = schedule_source["Déblocage détecté (€)"].fillna(0).round(2).reset_index(drop=True)
    schedule_drawdowns.name = "Déblocage détecté (€)"

    drawdown_records: list[dict[str, object]] = []
    cumulative_drawdowns = 0.0
    if opening_balance > 0.05:
        opening_row = source.loc[initial_mask].iloc[0]
        cumulative_drawdowns = opening_balance
        drawdown_records.append(
            {
                "Date": pd.to_datetime(opening_row["Date"], errors="coerce"),
                "N° ACD": _number(opening_row.get("N°")),
                "Déblocage détecté (€)": round(opening_balance, 2),
                "Capital cumulé débloqué (€)": round(cumulative_drawdowns, 2),
                "Solde après opération (€)": round(opening_balance, 2),
            }
        )
    for index, row in schedule_source.iterrows():
        drawdown = float(row["Déblocage détecté (€)"] or 0.0)
        if drawdown <= 0.05:
            continue
        cumulative_drawdowns += drawdown
        drawdown_records.append(
            {
                "Date": pd.to_datetime(row["Date"], errors="coerce"),
                "N° ACD": _number(row.get("N°")),
                "Déblocage détecté (€)": round(drawdown, 2),
                "Capital cumulé débloqué (€)": round(cumulative_drawdowns, 2),
                "Solde après opération (€)": round(_number(row.get("Restant Dû")) or 0.0, 2),
            }
        )
    drawdowns = pd.DataFrame(
        drawdown_records,
        columns=[
            ...
            "N° ACD",
            "Déblocage détecté (€)",
            "Capital cumulé débloqué (€)",
            "Solde après opération (€)",
        ],
    )
    capital = float(round(opening_balance + schedule_drawdowns.sum(), 2))
    ...
    reclassified_count = int(drawdown_in_other.sum())
    if reclassified_count:
        reclassified_total = float(other_amounts.loc[drawdown_in_other].sum())
        source_warnings.append(
            f"{reclassified_count} montant(s) de la colonne « Autre », pour {reclassified_total:,.2f} €, "
            "ont été reconnus comme des déblocages et non comme des frais."
        )
    unmatched_drawdowns = (drawdown_amounts > 0.05) & ~drawdown_in_other & (other_amounts.abs() > 0.05)
    if int(unmatched_drawdowns.sum()):
        source_warnings.append(
            f"{int(unmatched_drawdowns.sum())} augmentation(s) de dette ne correspondent pas exactement à la colonne « Autre ». "
            "Le déblocage a été calculé depuis la variation du restant dû et « Autre » a été conservé en frais."
        )
    if unexplained_decreases:
        source_warnings.append(
            f"{len(unexplained_decreases)} baisse(s) du restant dû ne sont pas entièrement expliquées par le capital remboursé."
        )
    annuity_difference = (
    ...
    return ConversionResult(
        ...
        schedule=schedule,
        drawdowns=drawdowns,
        schedule_drawdowns=schedule_drawdowns,
        opening_balance=round(opening_balance, 2),
        total_drawdowns=round(capital, 2),
...
def validate_pennylane_loan(
    summary: pd.DataFrame,
    schedule: pd.DataFrame,
    schedule_drawdowns: pd.Series | list[float] | None = None,
    ...
    if schedule_drawdowns is None:
        drawdowns = []
        previous_for_inference = opening_balance
        for _, row in schedule.reset_index(drop=True).iterrows():
            balance = _number(row.get("Solde (€)")) or 0.0
            principal = _number(row.get("Amortissement (€)")) or 0.0
            movement = balance - previous_for_inference + principal
            drawdowns.append(max(0.0, movement))
            previous_for_inference = balance
    else:
        drawdowns = [float(_number(value) or 0.0) for value in list(schedule_drawdowns)]
        if len(drawdowns) != len(schedule):
            add("Erreur", "Échéancier", "Déblocages", "Le nombre de déblocages techniques ne correspond pas aux lignes de l'échéancier.")
            drawdowns = (drawdowns + [0.0] * len(schedule))[: len(schedule)]

    detected_capital = round(float(opening_balance) + sum(drawdowns), 2)
    if capital is not None and abs(capital - detected_capital) > 0.05:
        add(
            "Erreur",
            "Paramètres",
            "Capital (€)",
            f"Le capital doit correspondre au total des déblocages détectés : {detected_capital:.2f} €.",
        )
    ...
    for index, row in schedule.reset_index(drop=True).iterrows():
        ...
        if previous_balance is not None and principal is not None and balance is not None:
            expected_balance = previous_balance + drawdowns[index] - principal
            if abs(balance - expected_balance) > 0.05:
                add(
                    "Erreur",
                    excel_row,
                    "Solde (€)",
                    f"Solde attendu : {expected_balance:.2f} € "
                    f"(solde précédent + déblocage {drawdowns[index]:.2f} € - amortissement).",
                )
            previous_balance = balance
